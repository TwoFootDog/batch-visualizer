...
import edu.project.authorization.util.CorsFilter;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests()
                    .antMatchers("/", "/signup","/signin", "/application/**").permitAll() // 모든 권한에 접근 허용
//                    .antMatchers("/**").hasRole("USER")     // USER 권한에 접근 허용
                    .antMatchers("/admin/**", "member/**").hasRole("ADMIN") // ADMIN 권한에 접근 허용
//                    .antMatchers("/auth/member/**", "/auth/getUserInfo").hasRole("USER")     // USER 권한에 접근 허용
//                    .antMatchers("/auth/member/**", "/auth/getUserInfo").hasAnyRole("ADMIN","USER")
